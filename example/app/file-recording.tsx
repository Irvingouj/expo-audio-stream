...
export default function FileRecordingScreen() {
  ...
  const [recordingResult, setRecordingResult] = useState<AudioRecording | null>(
    null
  );
  ...
  const [audioPlayerSource, setAudioPlayerSource] = useState<string | null>(
    null
  );

  const player = useAudioPlayer(audioPlayerSource);
  const expoAudioPlayerStatus = useAudioPlayerStatus(player);
  ...
  useEffect(() => {
    console.log("AudioPlayerSource:", audioPlayerSource);
  }, [audioPlayerSource]);
  ...
  const handleStartRecording = async () => {
    try {
      ...
      const { recordingResult: result, subscription } =
  ...
  const handleStopRecording = async () => {
    try {
      ...
      setAudioPlayerSource(recording.fileUri);
  ...
  const handlePlayRecording = async () => {
    ...
    if (!player.isLoaded) {
      Alert.alert("Error", "Audio player is not ready");
      return;
    }
  ...
  const isPlaying = useMemo(() => player.playing, [player.playing]);
    ...
    <ScrollView style={styles.container}>
      <View style={styles.content}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.buttonContainer}>
            <TouchableOpacity
              style={[
                styles.button,
                styles.playButton,
                (!recordingResult || isRecording || isPlaying) &&
          ...
          {expoAudioPlayerStatus && (
            <View style={styles.status}>
              <Text style={styles.statusText}>
                {JSON.stringify(expoAudioPlayerStatus, null, 2)}
              </Text>
            </View>
          )}
        </View>
        ...
        {recordingResult && (
          <View style={styles.info}>
            ...
            <Text style={styles.infoText}>
              • File: {recordingResult.filename}
            </Text>
            <Text style={styles.infoText}>
              • Duration: {(recordingResult.durationMs / 1000).toFixed(1)}s
            </Text>
            <Text style={styles.infoText}>
              • Size: {(recordingResult.size / 1024).toFixed(1)} KB
            </Text>
            <Text style={styles.infoText}>
              • Sample Rate: {recordingResult.sampleRate}Hz
            </Text>
            <Text style={styles.infoText}>
              • Channels: {recordingResult.channels}
            </Text>
            <Text style={styles.infoText}>
              • Bit Depth: {recordingResult.bitDepth}-bit
            </Text>
            <Text style={styles.infoText}>
              • MIME Type: {recordingResult.mimeType}
            </Text>
          </View>
        )}
        ...
        <View style={styles.info}>
          ...
          <Text style={styles.infoText}>
            • Method: ExpoPlayAudioStream.startRecording()
          </Text>
          <Text style={styles.infoText}>
            • Sample Rate:{" "}
          ...
          <Text style={styles.infoText}>
            • Output: Audio file with metadata
          </Text>
        </View>
      </View>
    </ScrollView>
