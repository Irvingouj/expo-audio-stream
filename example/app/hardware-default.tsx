...
export default function HardwareDefaultScreen() {
  ...
  const onAudioCallback = async (audio: AudioDataEvent) => {
    console.log('Hardware default sample rate:', audio.soundLevel);
    updateVolume(audio);
  };
  ...
  const handleStartRecording = async () => {
    try {
      ...
      console.log('Hardware default sample rate:', JSON.stringify(recordingResult, null, 2));
      eventListenerSubscriptionRef.current = subscription;
      setIsRecording(true);
      setRecordingStatus(JSON.stringify(recordingResult, null, 2));
      Alert.alert('Success', 'Microphone streaming started with hardware default sample rate');
    } catch (error) {
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Text style={styles.title}>Hardware Default Microphone Streaming</Text>
      <Text style={styles.description}>
        This test starts microphone streaming without specifying a sample rate,
        allowing the system to use the hardware's default sample rate.
      </Text>
      ...
      <View style={styles.info}>
        ...
        <Text style={styles.infoText}>• Sample Rate: Hardware Default (unspecified)</Text>
        ...
        <Text style={styles.infoText}>• Purpose: Test default audio configuration</Text>
      </View>

      {recordingStatus ? (
        <View style={styles.result}>
          <Text style={styles.resultTitle}>Hardware Default Streaming Result:</Text>
